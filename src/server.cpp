...
void handle_client(int client_sock, DB* db, const std::string& server_password) {
    char buffer[BUFFER_SIZE];
    std::string cmd, key, value;
    bool authenticated = server_password.empty(); // If no password is set, client is automatically authenticated
    ...
    while (true) {
        ...
        if (!authenticated) {
            if (cmd == "AUTH") {
                ...
                send(client_sock, "-ERR Authentication required. Use AUTH <password>\n", 50, 0);
            }
            continue;
        }
        ...
        if (cmd == "SET") {
            iss >> key;
            if (key.empty()) {
                send(client_sock, "Usage: SET key value\n", 21, 0);
                continue;
            }
            iss >> std::ws;
            ...
            iss >> key;
            if (key.empty()) {
                send(client_sock, "Usage: GET key\n", 15, 0);
                continue;
            }
            if (db->get(key, value)) {
            ...
            iss >> key;
            if (key.empty()) {
                send(client_sock, "Usage: DEL key\n", 15, 0);
                continue;
            }
            db->del(key);
            send(client_sock, "+OK\n", 4, 0);
            ...
            db->save("db.json");
            send(client_sock, "+SAVED\n", 7, 0);
        } else if (cmd == "LOAD") {
            db->load("db.json");
            send(client_sock, "+LOADED\n", 8, 0);
        } else {
            send(client_sock, "Unknown command. Type HELP (if authenticated).\n", 47, 0);
        }
    }
#ifdef _WIN32
...
int main() {
    ...
    while (true) {
        ...
        std::lock_guard<std::mutex> lock(db_swap_mutex);
        std::thread(handle_client, client_sock, db.get(), cfg.password).detach();
    }
#ifdef _WIN32
